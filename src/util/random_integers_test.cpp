...
TEST(GenRandomTest, ValidRange) {
  int min = 1, max = 10, count = 5;
  std::vector<int> numbers = GenRandom(count, min, max);

  ASSERT_EQ(numbers.size(), count);
  EXPECT_TRUE(AreNumbersInRange(numbers, min, max));
}

TEST(GenRandomTest, EmptyResultOnZeroCount) {
...
TEST(GenRandomTest, SingleNumberGeneration) {
